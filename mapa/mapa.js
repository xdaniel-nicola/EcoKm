    ...
    if (start && end) {
        ...
        fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(start)}.json?access_token=${mapboxgl.accessToken}`)
            ...
            .then(data => {
                ...
                document.getElementById('result').innerHTML = `Distância: ${distance.toFixed(2)} km<br>Duração: ${duration.toFixed(2)} min`;
            })
            .catch(error => {
                console.error("Erro:", error);
                alert("Ocorreu um erro ao traçar a rota. Verifique os endereços e tente novamente.");
            });
    }
});
